...
template <typename T>
inline void ExtractPatchIntoBufferColumnWithPadding(const RuntimeShape& input_shape, int w,
                                                    int h, int b, int kheight, int kwidth,
                                                    int stride_width, int stride_height,
                                                    int pad_width, int pad_height,
                                                    int in_width, int in_height,
                                                    ...
                                                    int buffer_id, const T* in_data,
                                                    T* conv_buffer_data, uint8 zero_byte) {
  ruy::profiler::ScopeLabel label("ExtractPatchIntoBufferColumnWithPadding");
  TFLITE_DCHECK_EQ(input_shape.DimensionsCount(), 4);
  // This chunk of code reshapes all the inputs corresponding to
  // output (b, h, w) to a column vector in conv_buffer(:, buffer_id).
  const int kwidth_times_indepth = kwidth * in_depth;
...
template <typename T>
void Im2colWithPadding(const ConvParams& params, int kheight, int kwidth, uint8 zero_byte,
                       const RuntimeShape& input_shape, const T* input_data,
                       const RuntimeShape& output_original_shape, 
                       const RuntimeShape& output_padded_shape, T* output_data) {
  ruy::profiler::ScopeLabel label("Im2colWithPadding");
  const int stride_width = params.stride_width;
  const int stride_height = params.stride_height;
  const int pad_width = params.padding_values.width;
  const int pad_height = params.padding_values.height;
  TFLITE_DCHECK_EQ(input_shape.DimensionsCount(), 4);
  TFLITE_DCHECK_EQ(output_original_shape.DimensionsCount(), 4);

  const int batches = MatchingDim(input_shape, 0, output_original_shape, 0);
  const int input_depth = input_shape.Dims(3);
  const int input_width = input_shape.Dims(2);
  const int input_height = input_shape.Dims(1);
  const int output_depth = output_original_shape.Dims(3);
  const int output_width = output_original_shape.Dims(2);
  const int output_height = output_original_shape.Dims(1);
  ...
  for (int b = 0; b < batches; ++b) {
    for (int h = 0; h < output_height; ++h) {
      for (int w = 0; w < output_width; ++w) {
        ExtractPatchIntoBufferColumnWithPadding(
            input_shape, w, h, b, kheight, kwidth, stride_width, stride_height,
            pad_width, pad_height, input_width, input_height, input_depth,
            output_depth, buffer_id, input_data, output_data, zero_byte);
        ++buffer_id;
