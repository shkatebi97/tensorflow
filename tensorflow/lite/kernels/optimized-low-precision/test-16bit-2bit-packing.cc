...
int main(){
    ...
    int ws[] = {  1,  1, -1 };
    std::cout << "a = { " 
              << as[0] << " (0b" << std::bitset<2>((as[0] & 0x03)) << "), "
              << as[1] << " (0b" << std::bitset<2>((as[1] & 0x03)) << "), "
              << as[2] << " (0b" << std::bitset<2>((as[2] & 0x03)) << ") }";
    int16_t a = 0 | ((as[0] & 0x03) << 0) | ((as[1] & 0x03) << 6) | ((as[2] & 0x03) << 12);
    std::cout << " -> 0b" << std::bitset<16>(a) << std::endl;
    std::cout << "w = { " 
              << ws[0] << " (0b" << std::bitset<2>((ws[0] & 0x03)) << "), "
              << ws[1] << " (0b" << std::bitset<2>((ws[1] & 0x03)) << "), "
              << ws[2] << " (0b" << std::bitset<2>((ws[2] & 0x03)) << ") }";
    int16_t w = 0 | ((ws[2] & 0x03) << 0) | ((ws[1] & 0x03) << 6) | ((ws[0] & 0x03) << 12);
    std::cout << " -> 0b" << std::bitset<16>(w) << std::endl;
    int32_t o                   = a * w;
    int32_t oOriginal           = as[0] * ws[0] + as[1] * ws[1] + as[2] * ws[2];
    uint32_t oOriginalUnsinged  = (((uint)as[0]) & 0x03) * (((uint)ws[0]) & 0x03) + 
                                  (((uint)as[1]) & 0x03) * (((uint)ws[1]) & 0x03) + 
                                  (((uint)as[2]) & 0x03) * (((uint)ws[2]) & 0x03);
