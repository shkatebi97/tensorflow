...
parser.add_option('-n', '--iterations',
    ...
    help="Set iteration count", default=1000)
...
methods_order = ['I8-I8', 'I8-I4', 'I4-I8', 'I4-I4', 'I8-Ternary', 'Ternary-I8', 'Ternary-Ternary', 'I8-Binary', 'Binary-I8', 'Binary-Binary', 'Binary-Binary-XOR']
metrics_order = [
    'cpu_cycles',
    'instructions',
    'l1d_loads',
...
def add_worksheet_to_workbook(
    workbook: object, sheet_name: str, table_names: list[str],
    x_values: list[str], y_values: list[str], c_value: int, 
    data: dict[dict[str]] | object | FunctionType, transform = lambda x: x, extra = None,
    transform_wrong_data = lambda x: x if x != 0 else 1
    ) -> object:
    groups = []
    output_list = []
...
def add_extra_min_max(payload: dict):
    worksheet, groups, simple_format, bold_italic_format = payload["worksheet"], payload["groups"], payload["simple_format"], payload["bold_italic_format"]
    new_groups = []
    for group in groups:
...
def add_colorscale_to_groups(payload: dict):
    worksheet, groups = payload["worksheet"], payload["groups"]
    for group in groups:
