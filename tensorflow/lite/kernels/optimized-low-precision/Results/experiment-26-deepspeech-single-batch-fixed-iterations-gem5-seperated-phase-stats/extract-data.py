...
def add_worksheet_to_workbook(
    ...
    if extra is not None:
        if extra_is_extras:
            output_row = []
            output_row.append((sheet_name, bold_italic))
            for z in range(num_extras):
                output_row.append(("", bold_italic))
                output_row.append(("Speed up against baseline" if extra_title[z] is None else extra_title[z], bold_italic))
            output_list.append(output_row)
        else:
    ...
    for i, y in enumerate(y_values):
        ...
        for j, x in enumerate(x_values):
            ...
            if extra is not None and extra_value is not None and not extra_is_extras:
            ...
            if extra is not None and extra_value is None and extra_is_extras:
                for z in range(num_extras):
                    output_row[j + (z + 1) * (num_columns + 1) + 1] = (extra[z](i, j, existing_worksheet_y_offset, num_rows, num_columns), simple, True)
        output_list.append(output_row)
    output_list.append([])
    groups.append("{0}{1}:{0}{2}".format(num_column_to_spreadsheet_letter(1), existing_worksheet_y_offset + 3, existing_worksheet_y_offset + 1 + num_rows))
    if extra is not None:
        if not extra_is_extras:
            extra_groups.append("{0}{1}:{0}{2}".format(num_column_to_spreadsheet_letter(1 + num_columns + 1), existing_worksheet_y_offset + 3, existing_worksheet_y_offset + 1 + num_rows))
        elif extra_is_extras:
            for z in range(1, num_extras + 1):
                extra_groups.append("{0}{1}:{0}{2}".format(num_column_to_spreadsheet_letter(1 + z * (num_columns + 1)), existing_worksheet_y_offset + 3, existing_worksheet_y_offset + 1 + num_rows))
    ...
    for i, row in enumerate(output_list):
        show_barrier = False
        if show_barrier_after is not None and i == show_barrier_after:
            show_barrier = True
        i += existing_worksheet_y_offset
        if len(row) == 1 + 2 * num_extras:
            barrier_format = copy_format(workbook, row[0][1])
            barrier_format.set_bottom(5)
            barrier_format.set_bottom_color('#000000')
            worksheet.merge_range(i, 0, i, len(x_values), row[0][0], row[0][1] if not show_barrier else barrier_format)
            for z in range(1, num_extras + 1):
                barrier_format = copy_format(workbook, row[z * 2][1])
                barrier_format.set_bottom(5)
                barrier_format.set_bottom_color('#000000')
                worksheet.merge_range(i, z * (len(x_values) + 2), i, z * (len(x_values) + 2) + len(x_values), row[z * 2][0], row[z * 2][1] if not show_barrier else barrier_format)
        elif len(row) == 1:
            barrier_format = copy_format(workbook, row[0][1])
            barrier_format.set_bottom(5)
            barrier_format.set_bottom_color('#000000')
            ...
            for j, cell in enumerate(row):
                if len(cell) == 3:
                    cell_value, cell_format, is_formula = cell
                    barrier_format = copy_format(workbook, cell_format)
                    barrier_format.set_bottom(5)
                    barrier_format.set_bottom_color('#000000')
                    if is_formula:
                        worksheet.write_formula(i, j, cell_value, cell_format if not show_barrier else barrier_format, "")
                    else:
                        worksheet.write(i, j, cell_value, cell_format if not show_barrier else barrier_format)
                elif len(cell) == 5:
                    cell_value, cell_format, is_formula, compare_to_next_row, compare_to_next_col = cell
                    new_cell_format = copy_format(workbook, cell_format)
                    if compare_to_next_row != 0:
                    ...
                    if compare_to_next_col != 0:
                        ...
                        if compare_to_next_col > 0:
                            ...
                            new_cell_format.set_bottom_color('#bf0041')
                    barrier_format = copy_format(workbook, new_cell_format)
                    barrier_format.set_bottom(5)
                    barrier_format.set_bottom_color('#000000')
                    if is_formula:
                        worksheet.write_formula(i, j, cell_value, new_cell_format if not show_barrier else barrier_format, "")
                    else:
                        worksheet.write(i, j, cell_value, new_cell_format if not show_barrier else barrier_format)
                else:
                    cell_value, cell_format = cell
                    barrier_format = copy_format(workbook, cell_format)
                    barrier_format.set_bottom(5)
    ...
    for z in range(1, num_extras + 1):
        worksheet.set_column(z * (len(x_values) + 2) - 1    , z * (len(x_values) + 2) - 1    , 2)
    if extra is not None:
        if not extra_is_extras:
            worksheet.set_column(len(x_values) + 2, len(x_values) + 2, max(list(map(len, y_values))) + 4)
        else:
            for z in range(1, num_extras + 1):
                worksheet.set_column(z * (len(x_values) + 2), z * (len(x_values) + 2), max(list(map(len, y_values))) + 4)
    for i in range(len(x_values)):
        ...
        if extra is not None:
            if not extra_is_extras:
                ...
                for z in range(1, num_extras + 1):
                    worksheet.set_column((z * (len(x_values) + 2)) + i + 1, (z * (len(x_values) + 2)) + i + 1, selected_len)
    baseline_extra = []
    if extra is not None:
        if not extra_is_extras:
            baseline_extra.append("{0}{1}:{2}{1}".format(num_column_to_spreadsheet_letter(1 + num_columns), existing_worksheet_y_offset + 3, num_column_to_spreadsheet_letter(1 + num_columns + 1)))
        else:
            for z in range(1, num_extras + 1):
                baseline_extra.append("{0}{1}:{2}{1}".format(num_column_to_spreadsheet_letter(z * (1 + num_columns)), existing_worksheet_y_offset + 2 + z, num_column_to_spreadsheet_letter(z * (1 + num_columns) + 1)))
    payload = {}
    payload["worksheet"] = worksheet
...
results_t = {}
for method in results[names[0]].keys():
    results_t[method] = {
        "DeepSpeech": results[names[0]][method]
    }
...
extras_titles = [
    ...
    "Speed-up against XOR",
]

remove_baseline_extra(
    add_colorscale_to_groups(
        add_worksheet_to_workbook(
            workbook, "Average", ["DeepSpeech"], methods, results_t, 
            extra=extras, extra_title=extras_titles, transform=lambda x: x, 
            extra_is_extras=True, show_barrier_after=methods_order.index("Binary-Binary-XOR") + 2
            # extra_value=lambda x, y: single_dimension_models_method_based_decrease_based_on_basline_val(results_t, y, x, 'I8-I8'),
            # existing_worksheet=payload["worksheet"], existing_worksheet_y_offset=payload["new_y_offset"],
        )
    )
)
...
for metric in metrics:
    if 'CPUCycles' == metric:
        extras[metric] = (
            ...
            lambda i, j, w_o, n_r, n_c: single_dimension_models_decrease_based_on_basline_str.format(num_column_to_spreadsheet_letter(1 + j).upper(), num_column_to_spreadsheet_letter(1).upper(), i + w_o + 3),
        ...
        extras[metric] = (
            ...
            lambda i, j, w_o, n_r, n_c: single_dimension_models_increase_based_on_basline_str.format(num_column_to_spreadsheet_letter(1 + j).upper(), num_column_to_spreadsheet_letter(1).upper(), i + w_o + 3),
