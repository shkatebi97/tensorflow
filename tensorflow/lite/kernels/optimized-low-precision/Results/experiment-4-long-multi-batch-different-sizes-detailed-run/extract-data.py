...
for result_method_dir in result_methods_dirs:
    ...
    with open(file_name) as file:
        ...
        models_size = list(map(lambda line: "{}x{}^{}".format(line.split("-")[0], line.split("x")[1].split(".tflite")[0], int(line.split("batch-")[1].split("x")[0]) + 1), lines[0::2]))
...
sizes = results.keys()
sizes = list(map(lambda x: (x, "{}-batch-{}x{}".format(x.split('x')[0], int(x.split('^')[1]) - 1, x.split('x')[1].split('^')[0])), sizes))

print_as_csv(
    "Average Run of each model for each method",
    "Sizes", methods, sizes, results
)

"""
print("".join(['-']*100))
print("Average Run of each model for each method")
print("".join(['-']*100))
print("Sizes", end=",")
...
for result_method_dir in result_methods_dirs:
    ...
    for size_tuple in sizes:
        ...
        with open(join(dir_name, report_file_name)) as report_f:
            ...
            report_lines = list(filter(lambda x: method_kernels[method] in x, report_lines))
