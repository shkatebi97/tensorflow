        ...
        "GEMM": {
            ...
            "Kernel": {
                ...
                "Overall": {
                    ...
                    "I4-I4": "LowPrecision::FullyConnected::Multiply",
                    "Ternary-Ternary": "LowPrecision::FullyConnected::Multiply",
                    "Binary-Binary": "LowPrecision::FullyConnected::Multiply",
                    "ULPPACK-W1A1": "LowPrecision::FullyConnected::Multiply",
                    "ULPPACK-W2A2": "LowPrecision::FullyConnected::Multiply",
                    "ULPPACK-W3A3": "LowPrecision::FullyConnected::Multiply",
                },
            },
            "Packing": {
                "HasChild": False,
                "Overall": {
                    ...
                    "I4-I4": "LowPrecision::FullyConnected::QuantizeInput",
                    "Ternary-Ternary": "LowPrecision::FullyConnected::QuantizeInput",
                    "Binary-Binary": "LowPrecision::FullyConnected::QuantizeInput",
                    "ULPPACK-W1A1": "LowPrecision::FullyConnected::QuantizeInput",
                    "ULPPACK-W2A2": "LowPrecision::FullyConnected::QuantizeInput",
                    "ULPPACK-W3A3": "LowPrecision::FullyConnected::QuantizeInput",
                },
            },
        },
        "Others": {
            ...
            figure_stacked_ybar_tex_template.format(
                ...
                PLOTS="\n                ".join(list(map(
                    lambda function_name: single_plot_tex_template.format(
                        COORDS=" ".join(
                            list(map(lambda method: f"({method},{functions_results[model][method][selected_functions[function_name][method]][0] * results[model][method]:.2f})" if method in functions_results[model] else f"({0:.2f},{method})", selected_methods))
                        ) if function_name != "Other Layers" else " ".join(
                            list(map(lambda method: f"({method},{(1 - functions_results[model][method][selected_functions[function_name][method]][0]) * results[model][method]:.2f})" if method in functions_results[model] else f"({0:.2f},{method})", selected_methods))
                        )
                    ),
...
for model in models_names:
    ...
    subprocess.run(["pdflatex", "-shell-escape", "-interaction=nonstopmode", "-file-line-error", f"-output-directory={str(detailed_comparison_figure_parent_path)}", join(detailed_comparison_figure_tex_parent_path, f"{model}.tex")], check=True, capture_output=True)
