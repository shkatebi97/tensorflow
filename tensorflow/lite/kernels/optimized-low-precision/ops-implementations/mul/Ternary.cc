...
void doTernaryMultiplication(int8_t* activation, 
                            int8_t* weights, 
...
void doTernaryMultiplication(float* activation, 
                            int32_t* weights, 
...
void doTernaryMultiplication(float16* activation, 
                            int16_t* weights, 
...
Status Mul::Prepare_ternary(DataType data_type){
        ...
        for (int i = 0 ; i < k_shape.size[1] ; i++){
            for (int j = 0 ; j < k_shape.size[0] ; j++){
                ...
                weights_packed[i * (k_shape.size[0]/8) + ((cluster_id * 16) + container_id)] |= 
                    ...
                    (
                        (weights_reference[i * k_shape.size[0] + j] > 0)?(0b01):(0b11)
                    ):(0b00)
                    << (shift_amount * 2);
            }
        }
...
Status Mul::Init_ternary(){return Status::Success;}

Status Mul::Free_ternary(DataType data_type){
    if (data_type == DataType::Int8){
...
Status Mul::Eval_ternary(int8_t* lhs,
                        int32_t* dst, 
...
void doTernaryMultiplication(int8_t* activation, 
    ...
    asm volatile(
        ...
        // CMLT T3, W, #0
        "cmlt v8.16b,  v0.16b, #0\n"
        "cmlt v10.16b, v1.16b, #0\n"
        "cmlt v11.16b, v2.16b, #0\n"
        "cmlt v12.16b, v3.16b, #0\n"
